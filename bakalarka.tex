...
\thesissetup{
    ...
    author        = Martin Maťko,
    gender        = m,
    advisor       = Mgr. Vít Stanislav,
    title         = {Mobilní klient na platformě Android pro aplikaci na učení anatomie},
    TeXtitle      = {Mobilní klient na platformě Android pro aplikaci na učení anatomie},
...
\chapter{Rozdiely medzi webovou a mobilnou aplikáciou}
Neustále rastúci počet mobilných zariadení núti tvorcov webových aplikácií prispôsobovať sa novým typom zariadení. Napríklad už v marci 2015 počet užívateľov pripojených na internet pomocou mobilných zariadení prevýšil počet pripojení z klasických stolových počítačov. https://www.comscore.com/Insights/Blog/Number-of-Mobile-Only-Internet-Users-Now-Exceeds-Desktop-Only-in-the-U.S

Z tohoto dôvodu upravil Google svoj vyhladávací algoritmus tak, aby sa medzi prvými výsledkami zobrazovali webstránky s vhodným dizajnom pre mobilné zariadenia. Medzi kritéria patrí napríklad dostatočne veľký font písma i na malom displeji alebo rozmiestenie prvkov na stránke tak, aby nebolo nutné horizontálne skrolovanie. Vytvorenie webstránky s responzívnym dizajnom tak priláka oveľa viac užívateľov, ktorí na surfovanie po internet používajú prevažne mobilné zariadenia. 

Výhodou webových aplikácií optimalizovaných pre mobilne zariadenia oproti natívnym aplikáciam sú: 
\begin{itemize}
  \item Bezprostrednosť - Aplikácie sú okamžite prítupné bez inštalovania do pamäti zariadenia
  \item Vyhľadateľnosť – Webové stránky sú jednoducho vyhľadatelné cey internetový vyhľadávač alebo v rôznych zoznamoch a katalógoch. Navyše, stránky môžu byť jednoducho zdieľané medzi užívateľmi ako aj rôznymi typmi zariadení.
  \item Čas a cena - Webové aplikácie sú jednoduchšie a lacnejšie na vývoj než natívne, najmä ak sa od aplikácie vyžaduje podpora rôznych operačných systémov
\end{itemize}

V niektorých prípadoch však webová aplikácia nestačí a je nutné zvážiť vytvorenie natívnej aplikácie pre mobilné zariadenia. Medzi hlavné výhody vytvorenia takejto aplikácie patrí:
\begin{itemize}
  \item Interaktivita/Hranie možnosť využitia hardvérových prvkov daného zariadenia, prispôsobenia ovládania, uloženie momentálneho stavu aplikácie
  \item Časté využívanie aplikácie možnosť rýchleho a jednoduchého spustenia aplikácie, nie je nutné najprv zapnúť prehliadač a až potom v nom spustiť aplikáciu, možnosť personalizácie 
  \item Podpora natívnych funkcií zariadenia  i napriek tomu, že webové prehliadače sa neustále zlepšujú pri využívaní fukncí mobilných zariadení, na prístup ku gyroskopu alebo kamere je stále lepšie a efektívnejšie využiť natívnu aplikáciu   
  \item Možnosť používania offline prístup ku obsahu alebo sprístupnenie niektorých funkcií je v natívnej aplikácií možná aj bez internetového pripojenia, pričom synchnronizácia a odoslanie dát na server sa vykoná pri najbližšom pripojení a užívateľ nie je blokovaný tým, že nemá prístup na internet
\end{itemize}
\chapter{Android}
Android je operačný systém momentálne vyvíjaný spoločnosťou Google. Založený je na linuxovom jadre a určený hlavne pre dotzkové obrazovky. Je využívaný v širokom spektre zariadení, od inteligentných hodiniek cez mobilné telefony a tablety až po automobily. Jeho zdrojový kod je šírený pod open source licenciou. Medzi jeho hlavné výhody patrí kompatibilita s obrovským množstvom zariadení a prisposobiteľnosť pre daného užívateľa.
\section{Vývoj aplikácií}
Vzhľadom na rôznorodosť mobilných zariadení je nutné venovať pri tvorbe mobilných aplikácií pozornosť veľkosti a orientácii obrazovky, výkonu zariadení ako aj užívateľsky prívetivému dotykovému ovládaniu. Pre tvorbu natívnych aplikácií si spoločnosť Google zvolila programovací jazyk Java. Tento jazyk je široko rozšírený, ponúka veľké množstvo knižníc, ako i vývojárskych nástrojov. Aplikácia potom beží na virtuálnom stroji, preto nie je nutné kompilovať aplikáciu pre každé zariadenie znova. 
\subsection{IDE}
Najpoužívanejším vývojovým prostredím je Android Studio, ktoré som si zvolil aj pre vývoj aplikácie Anatom. Toto prostredie je určené priamo pre vývojárov mobilných aplikácií a poskytuje veľké množstvo nástrojov pre jednoduchší a rýchlejší vývoj. Medzi hlavné prednosti určite patrí pokročilý dizajnér užívateľského rozhrania, na ktorom je možné zobraziť náhľad aplikácie pre rôzne veľkosti zariadení ako aj verzie operačného systému Android.
\subsection{Android Debug Bridge}
ADB je nástroj pre komunikáciu medzi testvaným zariadením a zariadením, na ktorom prebieha vývoj. Umožnuje kopírovanie súborov medzi zariadeniami, inštalovanie a odinštalovanie aplikácií, sledovanie správ od zariadenia, spúšťanie príkazov cez terminál a podobne. Testované zariadenie môže byť pripojené cez Wi-Fi alebo USB.
\subsection{Android Virtual Device}
Android Virtual Device (AVD) je emulátorová konfigurácia, ktorá umožňuje modelovať skutočné zariadenie tým, že definuje jeho hardvérové a softvérové vybavenie, ktoré je emulované Android Emulátorom.http://developer.android.com/tools/devices/index.html

Nové virtuálne zariadenia je možné pomocou nástroja AVD Manager. To poskytuje konfiguráciu vlastnstí zariadenia, ako je napríklad verzia operačného systému, procesor, veľkosť pamati RAM, rozlíšenie obrazovky, vnútorná pamať  a typ zariadenia. AVD Manager je predinštalovaný s niekoľkými profilmi skutočných zariadení, ako sú napríklad mobilné telefony Nexus. Za pomoci AVD je možné testovať a debuggovať danú aplikáciu aj bez toho, aby vývojár fyzicky vlastnil cieľové zariadenie. Ovládanie dotykom obrazovky je simulované myšou a je možné využiť aj hardvérovú klávesnicu zariadenia, na ktorom beží emulátor.
